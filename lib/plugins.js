...
export const loadPlugins = async (dir = path.join(process.cwd(), "plugins")) => {
  ...
  if (!fs.existsSync(dir)) {
    console.warn(`Woi folder '${dir}' ra onok co`);
    return plugins;
  }
    ...
    if (stat.isDirectory()) {
      ...
      try {
        ...
        if (typeof plugin === "function" && Array.isArray(plugin.command)) {
          ...
          console.warn(`Loh plugin '${file}' gak jelas iki (kudu function + .command).`);
        }
      } catch (err) {
        console.error(`Gagal load plugin nang ${filePath}:`, err);
      }
    }
  }
  ...
  for (const plugin of plugins) {
    if (plugin.command.map(c => c.toLowerCase()).includes(commandText.toLowerCase())) {
      try {
        ...
        console.error(`Waduh error pas mbukak plugin '${commandText}':`, err);
      }
      break;
    }
  }
...
export const getPluginStats = () => {
  ...
  for (const folder of folders) {
    try {
      ...
      console.error(`Gagal mbaca folder ${folder}:`, e.message);
    }
  }
